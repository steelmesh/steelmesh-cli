...
Mesh.prototype.connect = function(callback, allowCreate) {
    var dbname = this.meshdb,
        mesh = this,
        couch = nano(this.couchurl);
    ...
    couch.db.get(dbname, function(error, res) {
        if (res && res.error === 'not_found') {
            if (allowCreate) {
                ...
                callback('Cannot connect to ' + mesh.couchurl + '/' + mesh.meshdb, couch);
            } // if..else
        }
        else {
...
Mesh.prototype.getDbUrl = function() {
    return this.couchurl + '/' + this.meshdb;
};
...
Mesh.prototype.loadConfig = function(opts, callback) {
    ...
    fs.readFile(configFile, 'utf8', function(err, data) {
        ...
        mesh.couchurl = (opts.couchurl || config.couchurl || 'http://localhost:5984/').replace(reTrailingSlash, '');
        mesh.meshdb = opts.meshdb || config.meshdb || 'steelmesh';
        mesh.appid = opts.appid || config.appid || path.basename(mesh.targetPath);
...
Mesh.prototype.log = function(message, level) {
    (console[(level || '').toLowerCase()] || console.log)(message);
}; // log
